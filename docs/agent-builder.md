...
ClawShield currently requires users to already have Mako (or another OpenClaw-compatible agent) running. That's a significant barrier — most non-technical users have never heard of Mako, don't know what OpenClaw is, and shouldn't need to.

The goal: **a non-technical user should be able to go from zero to a personal AI assistant with security built in, entirely through the ClawShield UI.** No GitHub repos, no Docker knowledge, no understanding of agent frameworks.

---

## What This Feature Is

An in-UI agent builder that lets users create and configure a personal AI assistant without touching any external framework. ClawShield provisions, runs, and secures the agent — the user just answers questions.
...
│  ZeroClaw, or any       │    │  questions and ClawShield│
...
ClawShield turns this description into a system prompt automatically. The user never sees raw prompt text unless they click "Advanced."
...
Each option expands to show what ClawShield will restrict:
> *"Web browsing is HTTPS-only and blocked from accessing your local network. ClawShield scans every page it reads for hidden instructions."*
...
ClawShield generates a configuration and starts the agent. No files to save, no commands to run — the user clicks "Launch" and it starts.
...
The key difference from the old "Mode B" proof-of-concept: **the agent runs in its own isolated container**, not in-process with ClawShield. This gives it the same security posture as wrapping Mako.
...
┌──────────────────▼──────────────────────────────┐
│  clawshield-agent container (new)               │
│  - Generic secure agent runtime                 │
│  - Configured entirely by agent_config.json     │
│  - LLM calls routed through ClawShield proxy    │
...
The `clawshield-agent` container is a new minimal agent runtime image that ClawShield owns and maintains. It's not Mako — it's purpose-built to be:
- Configured entirely via `agent_config.json` (no env var archaeology)
- Hardened to the same level as the ClawShield container
...
When the user clicks "Launch," ClawShield:
...
The system prompt is auto-generated from the user's plain-English description (Step 2), with a base security prompt prepended by ClawShield:
...
| Risk | Mitigation |
|---|---|
| Agent runs in same process as ClawShield | Agent runs in its own container with no shared process space |
...
| Agent can make direct LLM calls bypassing proxy | DNS override: `api.anthropic.com` → ClawShield IP, same as Mode A |
| Workspace files as injection vector | ClawShield scans file content on read-back into LLM context |
| User-provided system prompt could be permissive | ClawShield prepends a non-overridable security preamble |

---

## New Components Needed

### `clawshield-agent` Docker image
...
- Routes all LLM calls through `http://clawshield:8000/proxy/{provider}`
- Writes audit log to `/app/audit/audit.log` in Mako's JSON-lines format (so `log_adapter.py` works unchanged)
- Published to `ghcr.io/clawshield/clawshield-agent:latest`
...
- The chat connects to the agent container's web endpoint (or ClawShield proxies it)
...
2. **Memory tool**: The agent-builder checkbox includes "Remember things" — this needs a concrete implementation. Simplest: append to `MEMORY.md` in the workspace volume. ClawShield injects it into the system prompt on each session.

3. **Docker-in-Docker**: If ClawShield itself is running in a container (the normal case), it can't run `docker compose up` to start the agent container. Options:
   - Mount the Docker socket (security risk — gives ClawShield root-equivalent access)
...
4. **clawshield-agent image**: Build as a subdirectory of this repo (`agent/Dockerfile`) or a separate repo? Separate repo is cleaner for publishing to GHCR independently.
